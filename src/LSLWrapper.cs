...
public static class LSLWrapper
{
	...
	/// <summary>
	/// The full StreamInfo of a connected inlet, extended description included.
	/// </summary>
	/// <remarks>
	/// Not the same object resolution hands back: a resolved <c>StreamInfo</c> carries only
	/// the header, so channel labels are absent from it and reading them there would always
	/// report none. This asks the sender.
	/// </remarks>
	public static object GetInletStreamInfo(object inlet, double timeout = 2.0)
	{
		try
		{
			var method = streamInletType.GetMethod("GetStreamInfo", [typeof(double)]);
			return method?.Invoke(inlet, [timeout]);
		}
		catch (Exception e)
		{
			GD.PrintErr($"❌ Failed to fetch inlet StreamInfo: {e.Message}");
			return null;
		}
	}

	/// <summary>
	/// Channel labels from a StreamInfo's description, or an empty array.
	/// </summary>
	/// <remarks>
	/// <para>The read counterpart of <see cref="SetChannelLabels"/>, walking the same
	/// <c>channels/channel/label</c> path. A producer that labels its channels with control
	/// addresses makes its stream self-describing, which is what lets this renderer map a
	/// pose by name instead of by position.</para>
	/// <para>Empty on anything unexpected, deliberately: an unlabelled stream is the normal
	/// case for every producer written before labels existed, and a malformed description
	/// must not cost the inlet. The caller falls back to the fixed channel layout.</para>
	/// <para><c>Child</c> and <c>NextSibling</c> return <c>XMLElement.Null</c> rather than
	/// throwing when nothing matches, and <c>IsEmpty</c> throws on an invalid handle — so
	/// the walk tests <c>IsNull</c>, never <c>IsEmpty</c>.</para>
	/// </remarks>
	public static string[] GetChannelLabels(object streamInfo)
	{
		...
		{
			var descMethod = streamInfoType.GetMethod("get_Description");
			object description = descMethod?.Invoke(streamInfo, null);
			if (description == null)
				return [];

			var xmlType = description.GetType();
			var child = xmlType.GetMethod("Child", [typeof(string)]);
			var childValue = xmlType.GetMethod("ChildValue", [typeof(string)]);
			var nextSibling = xmlType.GetMethod("NextSibling", [typeof(string)]);
			var isNull = xmlType.GetProperty("IsNull");
			if (child == null || childValue == null || nextSibling == null || isNull == null)
			{
				GD.PrintErr("GetChannelLabels: the XML element API is not what was expected");
				return [];
			}

			bool Missing(object element) =>
				element == null || (bool)isNull.GetValue(element);

			object channels = child.Invoke(description, ["channels"]);
			if (Missing(channels))
				return [];

			var labels = new List<string>();
			for (object channel = child.Invoke(channels, ["channel"]);
				 !Missing(channel);
				 channel = nextSibling.Invoke(channel, ["channel"]))
			{
				// A channel with no label ends the run rather than being skipped: labels
				// are positional, so carrying on past a gap would attribute every later
				// address to the wrong channel.
				if (childValue.Invoke(channel, ["label"]) is not string label
					|| label.Length == 0)
					break;
				labels.Add(label);
			}
